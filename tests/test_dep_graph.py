...
def test_auto_scan_nested_project_populates_every_module():
    ...
    Runs at ``max_depth=0`` (per-directory) because that is the granularity where
    nested attribution matters; the default is top-level packages."""
    report = run_auto_scan(NESTED_PROJECT, max_depth=0)
